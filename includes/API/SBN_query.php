...
class SBN_query {
    ...
    */
    private $server = "opac.sbn.it:2100/nopac";
    private $syntax = "unimarc";
    
    private $session = NULL;
    private $fields = array("wti" => "1=4",
                            "ibn" => "1=7",
                            "isn"   => "1=8",
                            "wja"   => "1=31",
                            "wpe"   => "1=1004",
    ...
    public function search($ccl_query) {
        $this->connect();
        ...
        if (!yaz_ccl_parse($this->session, $ccl_query, &$ccl_result)){
                die("The query could not be parsed.");
        } else {
            // fetch RPN result from the parser
            $rpn = $ccl_result["rpn"];
            // do the actual query
            yaz_search($this->session, "rpn", $rpn);
            ...
            if (yaz_hits($this->session) > 0){
                ...
                $result = yaz_record($this->session, 1, "string");
                print($result);
                echo "<br><br>";
                // the parsing functions will be introduced later
                if($this->syntax === "mab") {
                    $parsedResult = $this->parse_mab_string($result);
                } elseif($this->syntax === "unimarc"){
                    $parsedResult = $this->parse_unimarc_string($result);
                } else {}
    ...
    private function parse_unimarc_string($record){
        $ret = array();
    ...
    private function get_subfield_value($parts, $subfield_label){
        $ret = "";
    ...
    private function custom_trim(& $value, & $key){
        $value = trim($value);
    }
    
    private function parse_mab_string($record){
        $result = array();
    ...
    private function exchange_chars($rep_string){
