...
class auth_manager {
    ...
    function __construct() {
        ...
        $this->conn = new db_manager($_CONFIG['host'], 
                                    $_CONFIG['user'], 
                                    $_CONFIG['pass'],
                                    $_CONFIG['dbname']);
        $this->conn->connect();
    ...
    function auth_clean_expired() {
        ...
        $this->conn->delete($_CONFIG['table_sessioni'], 
                "creation_date + " . $_CONFIG['expire'] . " <= " . time());
/*
        mysql_query("
...
*/
        if (count($this->conn->getResult()) != 1) {
            return array(AUTH_NOT_LOGGED, NULL);
        } else {
    ...
    function auth_login($uname, $passw) {
        ...
        $this->conn->select($_CONFIG['table_utenti'],"*",
                "username='" . $uname . "' and password=MD5('" . $passw . "')");
        /*
        $result = mysql_query("
	SELECT *
